...
const StatementSchema: Mongoose.Schema<StatementMaster> = new Mongoose.Schema<StatementMaster>({
    ...
    pid: { required: true, type: Mongoose.Schema.Types.ObjectId, select: true },
    referenceFileId: { required: false, type: Mongoose.Schema.Types.ObjectId, select: true, sparse: true },
    location: { type: Number, required: true, select: true },
    indicators: { type: Array<String | Number>, required: false },
    originalLine: { type: String, required: true, select: true },
    ...
    annotatedLine: { type: String, required: false, select: true, sparse: true },
    businessName: { type: String, required: false, select: true, sparse: true }
});
