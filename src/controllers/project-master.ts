...
pmRouter.use("/", (request: Request, response: Response, next: NextFunction) => {
    ...
    .get("/get-project", async function (request: Request, response: Response) {
        ...
        try {
            ...
            try {
                let _id: Mongoose.Types.ObjectId = new Mongoose.Types.ObjectId(<string>request.params.wid);
                let workspaceMaster = await appService.workspaceMaster.aggregateOne([{ $match: { _id } }]);
                if (!workspaceMaster) return response.status(404).end();
                let extractPath: string = <string>workspaceMaster.dirPath;
